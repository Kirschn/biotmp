...
function biotmp(token) {
    queue.process('message_send', function (job, done) {
        ...
        discordClient.channels.get(String(job.data.channel_id)).send(job.data.message, JSON.parse(job.data.options), function(error, message) {
            if (!error) {
                done();
            } else {
                done(error);
